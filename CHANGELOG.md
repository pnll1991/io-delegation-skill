...
Added Jev-guided cheap-first compute orchestration for projects with an approved semantic worker. Local rules retain T0 deterministic/targeted work and T2 security/debugging/architecture/editing work; Jev scores only bulk factual candidates for T1 cheap-worker suitability. One validated T1 attempt may use an operator-approved `compute_profiles.cheap` model/effort on the same adapter, and any failed, incomplete or unknown evidence escalates to the principal. The existing unconditional `context_auto_dispatch` experiment remains separate. System accounting now includes reported Jev routing/orchestration tokens and marks missing control-plane usage incomplete rather than free. No production cost-savings claim is made until the new path has paired live evidence.
