...
Added optional project-scoped Jev-guided conversation compaction for Claude Code, Codex and Cursor. Claude uses the vendored `tamaratran/fast-jev-compaction` core at pinned upstream commit `e3f262a7f4d42bd8dd32ced30d26176f7cb545b0` as a native `session.compact` replacement. Codex and Cursor use a stdlib Python bridge that journals tool I/O locally, asks Jev what exact tool evidence must survive native compaction, and rehydrates that evidence through each host's supported lifecycle hooks. This remains a separate opt-in from Jev routing because its data boundary is broader. Codex/Cursor never send full tool-result bodies to Jev; failures remain fail-open to native compaction.
