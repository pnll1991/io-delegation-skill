...
Added host-aware Jev model orchestration for approved Codex/Cursor CLI workers. Jev still emits metadata-only task scores; local `model_policy` now selects an approved profile using host-specific cost/capability/latency evidence and a project preset (`cost`, `balanced`, or `quality`). The default Codex registry is capped at Astra low; the default Cursor balanced ladder is Luna medium -> Luna high -> Sol medium -> Sol high and intentionally excludes Astra. Users can replace registries/orders, lower ceilings, block profiles, bound escalations, or explicitly use a reviewed Cursor native-Router model string. Valid-but-incomplete evidence may escalate; transport/config/budget failures return directly to the principal. `host-cli` reuses each host's existing authenticated CLI without copying credentials. The legacy single `compute_profiles.cheap` path remains compatible.
