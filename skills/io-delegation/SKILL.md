...
Si existe un router Jev explícitamente aprobado, consultalo solo después de localizar y solo cuando la ruta siga siendo ambigua. `current_rules`, error o baja confianza vuelven a esta tabla; `bulk_read` tampoco autoriza un worker. Para configurarlo o evaluarlo, leé [TYPESAFE_ROUTER.md](references/TYPESAFE_ROUTER.md) bajo demanda.
