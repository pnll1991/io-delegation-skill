...
class ValidatorTests(unittest.TestCase):
    ...
    def test_html_seo_and_security(self):
        ...
        self.assertTrue(sec['patterns']['eval']['present'])

    def test_scope_filters_gold_to_authorized_corpus(self):
        sec=mod.security_patterns(self.root,prefixes=['src'])
        self.assertTrue(sec['patterns']['ipc_renderer_invoke']['present'])
        self.assertFalse(sec['patterns']['eval']['present'])
        self.assertFalse(sec['patterns']['ipc_main_handle']['present'])
        counts=mod.extension_counts(self.root,prefixes=['src'],files=['package.json'])
        self.assertEqual(counts['tracked_files'],3)
        self.assertNotIn('other/unsafe.ts',mod.tracked(self.root,['src'],['package.json']))
